...
[![Skills](https://img.shields.io/badge/skills-109-blue)](https://github.com/GarethManning/claude-education-skills)
...
An open-source library of 109 evidence-based pedagogical skills for curriculum design, lesson planning, and assessment — usable today by any educator with access to Claude, and engineered for AI agent orchestration.
...
Connect via the MCP server to access all 109 skills in any Claude conversation — add `https://mcp-server-sigma-sooty.vercel.app/mcp` under **Settings → Connectors**. Skills activate when your conversation matches their topic. A dedicated Skills Directory listing is in progress.
...
109 skills across 14 domains. Each skill encodes a specific, evidence-grounded instructional or curriculum design decision. Works standalone today.
...
The skill library is available as a live MCP server. Any MCP-compatible client can discover, search, and invoke all 109 skills programmatically.
...
The server exposes:
- **111 tools** (109 skills + 4 discovery tools: `list_skills`, `find_skills`, `suggest_skills`, `get_skill_details`)
